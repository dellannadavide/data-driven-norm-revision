...
/**
 * simulation.MinDistNorm
 *
 */	
public class MinDistNorm extends SimpleDNFNorm {
	/**
    ...
    public static final String PROH_DIST = "dist";
    ...
    public MinDistNorm(String id, Random a) {
    	...
    	hm_proh.put(this.PROH_DIST, ""+ Utils.uniform_discr(a,this.MIN_DIST, this.MAX_DIST));
     ...
     * @param dead
     */
    public MinDistNorm(String id, Disjunct cond, Disjunct proh, Disjunct dead, Random r) {
    	super(id,cond, proh, dead, r);
    }
	public MinDistNorm(String id, List<Disjunct> cond, List<Disjunct> proh, List<Disjunct> dead, Random r) {
		super(id, cond, proh, dead, r);
	}
    
    /**
     * Case of simple2 norm, with one proposition per component
     * 
     */
    public MinDistNorm(String id, String cond, String proh, String dead, Random r) {
    	super(id, r);
    	List<Disjunct> c = new ArrayList<>();
		...
		h_proh.put(this.PROH_DIST, proh);
	...
	public double getDist() {
    	return Double.parseDouble(this.getSimpleProhibition().getLiterals().get(this.PROH_DIST));
    }
    	...
    	switch(comp_type) {
	    	...
	    	case "proh":
	    		for(int j=0;j<proh.size();j++) {
	    			...
	    			Set<String> keys = literals.keySet();
	    			for(String k:keys){
	    				if(k.equals(this.PROH_DIST)) {
	    					if(state.dist>Double.parseDouble(literals.get(PROH_DIST)))
		    					return false;
	    	            }
	    			}
				}
	...
	public Set<LinkedHashMap<String, String>> buildConjMoreSpecCond(Set<String> CS_prop) {
		...
		for (Set<String> gen_s : power) {
			LinkedHashMap<String, String> new_literals = new LinkedHashMap<String, String>(cond_literals);
...
//
//        //1. Take CS, set of states s from TN traces s.t. φ_C holds in s is followed by at least one state s’ where φ_P holds before the deadline.
//        Set<State> CS = new HashSet<State>();
//        for(Trace t : violatingTraces) {
//            CS.addAll(this.getDetachmentStates(t));
//        }
//        //System.out.println(CS);
//        //2. Let PHI = set of possible conjunctions from prop(CS) \ prop(φ_C).
//        Set<String> CS_prop = new HashSet<String>();
//        for(State s: CS) {
//            CS_prop.addAll(this.getConditionRelatedProp(s)); //note that for example in a state with km_2 there is no explicit "km_1" prop
//        } //prop(CS)
//        CS_prop.removeAll(this.getConditionProp()); //prop(CS) \ prop(φ_C)
//        /*System.out.println("Cond weakening. Prop(CS) - prop(phi_C) = ");
//        System.out.println(CS_prop);*/
//
//        /*HERE I ADD THE POLINOMIAL BOUNDING
//         * what do I do? I pick in CS_prop, at most 5 more specific positions, instead of all possible oneS
//         * is it useful in our small domain? yes because for example if cond is km1, dead km10 and the proh is violated at km9 we have all val from 1 to 10 just for the condition
//         * -
//         * I go through CS_prop, I select all the ones starting with km, I sort them, i take the first 4+1(the curr one)
//         * */
//        Set<String> bounded_CS_prop = new HashSet<String>();
//        ArrayList<Integer> km = new ArrayList<Integer>();
//        for(String s : CS_prop)
...
//
//        //3. Possible weakened conditions: {φ_C ∧ φ |φ in Φ}
//
//
//        moreSpecificConditions.add(new Disjunct(cond_literals)); //one of the new weaening is also the current one
//
//        //for(String pos : getKMoreSpecificPos(cond_literals.get(this.COND_POS), this.MAX_POS+"", 5)) { //it's a simplification for now. we are assuming that these positions belong to the states where the condition hold before the deadline
//        //this above commented now is replaced by the part above
//        //NOTE HOWEVER THAT NOW IN POWER THERE MAY BE SOME "INVALID" SETS, WHICH I WANT TO correct, AND THIS MAY LEAD TO DUPLICATES
//        //FOR EXAMPLE the TWO sets {km1, km3} and {km2, km3}, are both equivalent to {km3} in our domain.
//        //so i need to correct on the way finding the max
//        for (Set<String> gen_s : power) {
...
//    }
	...
	public Set<LinkedHashMap<String, String>> buildConjMoreSpecProh(Set<String> PS_prop) {
		...
		int c = 0;
		for(int i=0;i<distances.size() && c<this.SPACE_PARAM;i++) {
			int dist = distances.get(i);
			if(dist>Integer.parseInt(curr_proh_literals.get(this.PROH_DIST))) {
				bounded_PS_prop.add(dist+"");
				c++;
			}
		}
		if(bounded_PS_prop.size()>0)
			bounded_PS_prop.add(curr_proh_literals.get(this.PROH_DIST));
		...
		for (Set<String> gen_s : power) {
			...
			int i = new_literals.keySet().size(); //assuming there is always only one dist, the remaining are the appl
			int BIG_M = this.MAX_DIST + 1000;
			int min_dist = BIG_M;
			for(String str : gen_s) {
				try {
			...
			if(min_dist<BIG_M) {
				new_literals.put(this.PROH_DIST, min_dist+"");
				builtConj.add(new_literals);
			}
		}
		return builtConj;
...
//
//        //2. Let PHI = set of possible conjunctions from prop(PS) \ prop(φ_P)
//        Set<String> PS_prop = new HashSet<String>();
//        for(State s: PS) {
//            PS_prop.addAll(this.getProhibitionRelatedProp(s)); //I get both type and dist. //note that for example in a state with dist_10 there is no explicit "dist_9", dist_8, dist_7, etc prop, but as for the km they are implicit
//        }
//        PS_prop.removeAll(this.getProhibitionProp()); //prop(PS) \ prop(φ_P). I remove the current ones
//        /*System.out.println("Proh weakening. prop(PS) - prop(phi_P) = ");
//        System.out.println(PS_prop);*/
//
//
//        /*HERE I ADD THE POLINOMIAL BOUNDING
//         * I pick in PS_prop, at most 5 more specific dists (which means bigger dists), instead of all possible ones
//         * -
...
//            if(min_dist<BIG_M) {
//                //I add also the dist
//                new_literals.put(this.PROH_DIST, min_dist+""); //replace the current one with the new speed
//                //add the new proh
...
//    	Set<String> CPS_prop = new HashSet<String>();
//    	for(State s: CPS) {
//    		CPS_prop.addAll(this.getDeadlineRelatedProp(s));
//    	}
//    	CPS_prop.removeAll(this.getDeadlineProp());
...
//
	...
	public Set<LinkedHashMap<String, String>> buildConjLessSpecCond(Set<String> OPS_prop) {
		...
		Set<Set<String>> power = Sets.powerSet(bounded_OPS_prop);
		for (Set<String> gen_s : power) {
			LinkedHashMap<String, String> new_literals = new LinkedHashMap<String, String>();
			int i = 1;
			int max_km = -1;
			...
			if(max_km>-1) {
				...
				if(new_literals.values().size() == 0 ||
					...
					new_literals.put(this.COND_POS, "km"+max_km);
					builtConj.add(new_literals);
				}
			}
		}
...
//    	Set<String> OPS_prop = new HashSet<String>();
//    	for(State s: OPS) {
//    		OPS_prop.addAll(this.getConditionRelatedProp(s));
//    	}//OPS_prop containts all propositions in the OPS states, in particular the propositions about position and type
//
...
//    }
	...
	public Set<LinkedHashMap<String, String>> buildConjLessSpecProh(Set<String> IPS_prop) {
		...
		for(String str : IPS_prop) {
			try {
				int possible_dist = Integer.parseInt(str);
				if(possible_dist<=this.MAX_DIST)
					dist.add(possible_dist);
				else
					dist.add(this.MAX_DIST);
			} catch (NumberFormatException nfe) {
				bounded_IPS_prop.add(str);
			}
		}
		if(dist.size()>this.SPACE_PARAM) {
		...
		if(bounded_IPS_prop.size()>0)
			bounded_IPS_prop.add(proh_literals.get(this.PROH_DIST));
		...
		for (Set<String> gen_s : power) {
			...
			if(min_dist<BIG_M) {
				...
				curr_literals.remove(this.PROH_DIST);
				if(new_literals.values().size() == 0 ||
						(curr_literals.values().containsAll(new_literals.values()))) {
					new_literals.put(this.PROH_DIST, min_dist+""); //replace the current one with the new speed
					builtConj.add(new_literals);
...
//    	Set<String> IPS_prop = new HashSet<String>();
//    	for(State s: IPS) {
//    		//System.out.println(this.getProhibitionRelatedProp(s));
//    		IPS_prop.addAll(this.getProhibitionRelatedProp(s));
//    	}
...
//    		try {
//    	        int possible_dist = Integer.parseInt(str);
//    	        //System.out.println(possible_dist);
//    	        //if here it is actually the speed and not the type
//    	        //i also filter possible dist out of the range
//    	        if(possible_dist<=this.MAX_DIST)
//    	        	dist.add(possible_dist);
//    	        else
//    	        	dist.add(this.MAX_DIST);
...
//    	Set<String> DS_prop = new HashSet<String>();
//    	for(State s: DS) {
//    		DS_prop.addAll(this.getDeadlineRelatedProp(s));
//    	}
//    	DS_prop.removeAll(this.getDeadlineProp());
...
//    }
	...
	protected void consistencyCheck() {
		...
		if(Integer.parseInt(cond_pos.replace("km",""))>=Integer.parseInt(dead_pos.replace("km","")))
			...
			hm_proh_temp.remove(this.PROH_DIST);
